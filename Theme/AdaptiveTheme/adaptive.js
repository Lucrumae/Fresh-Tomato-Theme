/* adaptive.js - animation-smooth v3
   Nav hover: GPU-composited will-change:transform, spring easing, no reflow
   SP=cubic-bezier(0.34,1.56,0.64,1) SM=cubic-bezier(0.25,0.46,0.45,0.94)
*/
...
(function () {
    ...
    function _injectNavStyle(){
        if(document.getElementById('ft-nav-glass')) return;
        var navStyle=document.createElement('style');
        navStyle.id='ft-nav-glass';
        navStyle.textContent='td#navi a,#navi a{display:block;margin:2px 5px;padding-left:5px;border:none;outline:1px solid transparent;outline-offset:-1px;border-radius:6px;transition:background 0.14s cubic-bezier(0.25,0.46,0.45,0.94),outline-color 0.14s cubic-bezier(0.25,0.46,0.45,0.94),filter 0.14s cubic-bezier(0.25,0.46,0.45,0.94);}td#navi a:hover,#navi a:hover{background:rgba(255,255,255,0.13)!important;outline-color:rgba(255,255,255,0.22)!important;filter:brightness(1.18)!important;}td#navi a:active,#navi a:active{filter:brightness(0.96)!important;transition-duration:0.08s!important;}td#navi a.indent1.active,#navi a.indent1.active,#navi:not(:hover) a.indent1.active,td#navi:not(:hover) a.indent1.active{outline-color:currentColor!important;background:rgba(255,255,255,0.08)!important;filter:brightness(1.55)!important;color:inherit!important;}td#navi a.indent2.active,#navi a.indent2.active{outline-color:rgba(255,255,255,0.28)!important;background:rgba(255,255,255,0.08)!important;filter:brightness(1.55)!important;color:inherit!important;}td#navi a.indent2.active::after,#navi a.indent2.active::after,td#navi:not(:hover) a.indent1.active::after,#navi:not(:hover) a.indent1.active::after{display:none!important;}td#navi a.indent1:has(+div a.indent2.active),#navi a.indent1:has(+div a.indent2.active),td#navi a.indent1.ft-sub-active,#navi a.indent1.ft-sub-active{outline-color:rgba(255,255,255,0.28)!important;background:rgba(255,255,255,0.08)!important;filter:brightness(1.55)!important;}';
        document.head.appendChild(navStyle);
        // Set ft-sub-active pada indent1 yang submenu-nya aktif (mis: Misc)
        (function(){
            var links=document.querySelectorAll('td#navi a.indent1, #navi a.indent1');
            for(var i=0;i<links.length;i++){
                var next=links[i].nextElementSibling;
                var hasSub=next&&next.tagName==='DIV'&&next.querySelector('a.indent2.active');
                links[i].classList.toggle('ft-sub-active',!!hasSub);
            }
        })();
    }
    ...
    try{var cached=sessionStorage.getItem(CACHE_KEY);if(cached){var c=cached.split(",");applyPalette(parseInt(c[0]),parseInt(c[1]),parseInt(c[2]));}}catch(e){}
    var sampled=false;
    function sampleFrame(){try{var cv=document.createElement("canvas");cv.width=96;cv.height=54;var ctx=cv.getContext("2d");ctx.drawImage(vid,0,0,96,54);var px=ctx.getImageData(0,0,96,54).data,r=0,g=0,b=0,n=0;for(var i=0;i<px.length;i+=4){var pr=px[i],pg=px[i+1],pb=px[i+2],br=(pr+pg+pb)/3;if(br<15||br>240)continue;r+=pr;g+=pg;b+=pb;n++;}if(n===0)return false;var dr=Math.round(r/n),dg=Math.round(g/n),db=Math.round(b/n);applyPalette(dr,dg,db);try{sessionStorage.setItem(CACHE_KEY,dr+","+dg+","+db);}catch(e){}return true;}catch(e){return false;}}
    function trySample(){if(sampled)return;if(vid.readyState>=2){if(sampleFrame())sampled=true;}}
    vid.addEventListener("loadeddata",trySample);
    vid.addEventListener("canplay",trySample);
    vid.addEventListener("playing",trySample);
    var retryCount=0;
    var retryTimer=setInterval(function(){if(sampled||retryCount>15){clearInterval(retryTimer);return;}trySample();retryCount++;},200);
    ...
    function showUnmutePopup(){
        ...
        pStyle.textContent =
            '#ft-unmute-popup{' +
                ...
                'animation:ft-popup-in 0.48s cubic-bezier(0.34,1.56,0.64,1) both;' +
            '}' +
            '@keyframes ft-popup-in{' +
                'from{opacity:0;transform:translateX(-50%) translateY(22px) scale(0.86);}' +
                'to{opacity:1;transform:translateX(-50%) translateY(0) scale(1);}' +
            '}' +
            '#ft-unmute-popup.ft-popup-out{' +
                'animation:ft-popup-out 0.22s cubic-bezier(0.4,0.0,1,1) forwards;' +
            '}' +
            '@keyframes ft-popup-out{' +
                'from{opacity:1;transform:translateX(-50%) translateY(0) scale(1);}' +
                'to{opacity:0;transform:translateX(-50%) translateY(14px) scale(0.90);}' +
            '}';
    ...
    cStyle.textContent = '#ft-controls{position:fixed;bottom:16px;right:16px;z-index:9999;display:flex;gap:8px;opacity:0;transform:translateY(12px) scale(0.90);transition:opacity 0.26s cubic-bezier(0.25,0.46,0.45,0.94),transform 0.34s cubic-bezier(0.34,1.56,0.64,1);pointer-events:none;will-change:transform,opacity;}#ft-controls.visible{opacity:1;transform:translateY(0) scale(1);pointer-events:auto;}#ft-controls button{width:34px;height:34px;border-radius:50%;border:none;background:rgba(12,12,12,0.72);backdrop-filter:blur(14px) saturate(1.5);-webkit-backdrop-filter:blur(14px) saturate(1.5);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;transition:background 0.18s cubic-bezier(0.25,0.46,0.45,0.94),transform 0.24s cubic-bezier(0.34,1.56,0.64,1),box-shadow 0.20s cubic-bezier(0.25,0.46,0.45,0.94);box-shadow:0 3px 14px rgba(0,0,0,0.60),0 1px 0 rgba(255,255,255,0.08) inset;will-change:transform,background;}#ft-controls button:hover{background:rgba(50,50,50,0.92);transform:scale(1.22) translateY(-1px);box-shadow:0 6px 22px rgba(0,0,0,0.65),0 1px 0 rgba(255,255,255,0.12) inset;}#ft-controls button:active{transform:scale(0.90);transition-duration:0.08s;}#ft-controls button svg{width:16px;height:16px;fill:none;stroke:#fff;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;}body.ft-panel-hidden #container,body.ft-panel-hidden #navi,body.ft-panel-hidden #footer{opacity:0!important;pointer-events:none!important;transition:opacity 0.38s cubic-bezier(0.25,0.46,0.45,0.94);}body:not(.ft-panel-hidden) #container,body:not(.ft-panel-hidden) #navi,body:not(.ft-panel-hidden) #footer{opacity:1;transition:opacity 0.42s cubic-bezier(0.0,0.0,0.2,1);}';
    ...
    var icons = {
        ...
        changeBg:'<svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>'
    };
    ...
    // -- CHANGE BACKGROUND BUTTON (tidak muncul di login page) ---
    var isLoginPage = window.location.pathname.indexOf('login') !== -1;
    var btnChangeBg = null;
    if(!isLoginPage){
        btnChangeBg = document.createElement('button');
        btnChangeBg.id = 'ft-btn-changebg';
        btnChangeBg.innerHTML = icons.changeBg;
        btnChangeBg.title = 'Change background';
        btnChangeBg.addEventListener('click', function(){ showChangeBgDialog(); });
    }

    controls.appendChild(btnPanel);
    controls.appendChild(btnMute);
    if(btnChangeBg) controls.appendChild(btnChangeBg);
    ...
    function attachControls(){
        ...
        // Tanpa throttle: mousemove fire 100-300x/detik, tiap call getBoundingClientRect()
        // paksa browser layout reflow → halaman log/realtime jadi freeze
    ...
    if(sessionStorage.getItem('ft_show_panel_tip')){
        ...
        setTimeout(function(){
            ...
            tipStyle.textContent =
                ...
                'animation:ftTipIn 0.46s cubic-bezier(0.34,1.56,0.64,1) both;}' +
                ...
                '@keyframes ftTipIn{from{opacity:0;transform:translateY(14px) scale(0.90);}to{opacity:1;transform:translateY(0) scale(1);}}' +
                '.ft-tip-pulse{animation:ftPulse 1.6s cubic-bezier(0.25,0.46,0.45,0.94) infinite!important;}' +
            ...
            function removeTip(){
                tip.style.transition = 'opacity 0.24s cubic-bezier(0.4,0.0,1,1), transform 0.24s cubic-bezier(0.4,0.0,1,1)';
                tip.style.opacity = '0'; tip.style.transform = 'translateY(10px) scale(0.92)';
                setTimeout(function(){
    ...
    function showAlertDialog(msg){
        ...
        s.textContent =
            '#ft-alert-bd{position:fixed;inset:0;z-index:99998;' +
                ...
                'transition:background .34s cubic-bezier(0.25,0.46,0.45,0.94),backdrop-filter .34s cubic-bezier(0.25,0.46,0.45,0.94),-webkit-backdrop-filter .34s cubic-bezier(0.25,0.46,0.45,0.94)}' +
            ...
            '#ft-alert h3{font-size:17px;font-weight:700;color:#f0ece8;margin:0 0 7px;letter-spacing:-.01em;' +
                'opacity:0;transform:translateY(6px);transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .22s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .22s}' +
            ...
            '#ft-alert p{font-size:12px;color:rgba(240,236,232,.55);margin:0 0 26px;line-height:1.6;' +
                'opacity:0;transform:translateY(6px);transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .29s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .29s;' +
            ...
            '#ft-alert-ok{width:100%;padding:12px;border:none;border-radius:12px;' +
                ...
                'transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .35s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .35s,filter .18s,box-shadow .20s cubic-bezier(0.25,0.46,0.45,0.94);outline:none;will-change:transform}' +
            '#ft-alert.ft-in #ft-alert-ok{opacity:1;transform:translateY(0)}' +
            '#ft-alert-ok:hover{filter:brightness(1.12);transform:translateY(-2px);box-shadow:0 7px 22px rgba(0,0,0,.38)}' +
            '#ft-alert-ok:active{transform:scale(.95);transition-duration:.08s}';
    ...
    function showRebootDialog(){
        ...
        s.textContent =
            ...
            '#ft-dlg h3{font-size:17px;font-weight:700;color:#f0ece8;margin:0 0 7px;' +
                ...
                'transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .22s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .22s}' +
            ...
            '#ft-dlg p{font-size:12px;color:rgba(240,236,232,.38);' +
                ...
                'opacity:0;transform:translateY(6px);' +
                'transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .29s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .29s}' +
            '#ft-dlg.ft-in p{opacity:1;transform:translateY(0)}' +
            '#ft-dlg-row{display:flex;gap:8px;' +
                'opacity:0;transform:translateY(8px);' +
                'transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .35s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .35s}' +
            ...
            '#ft-dlg-no{flex:1;padding:12px;' +
                ...
                'transition:background .16s cubic-bezier(0.25,0.46,0.45,0.94),color .16s cubic-bezier(0.25,0.46,0.45,0.94),transform .22s cubic-bezier(0.34,1.56,0.64,1),box-shadow .20s cubic-bezier(0.25,0.46,0.45,0.94);outline:none;will-change:transform}' +
            '#ft-dlg-no:hover{background:rgba(255,255,255,.12);color:rgba(240,236,232,.96);transform:translateY(-2px);box-shadow:0 5px 16px rgba(0,0,0,.32)}' +
            '#ft-dlg-no:active{transform:scale(.95);transition-duration:.08s}' +
            '#ft-dlg-yes{flex:1;padding:12px;border:none;border-radius:12px;' +
                ...
                'transition:opacity .16s,transform .22s cubic-bezier(0.34,1.56,0.64,1),box-shadow .20s cubic-bezier(0.25,0.46,0.45,0.94),filter .16s;outline:none;will-change:transform}' +
            '#ft-dlg-yes:hover{opacity:.92;transform:translateY(-2px);filter:brightness(1.12);box-shadow:0 7px 24px rgba(0,0,0,.38)}' +
            '#ft-dlg-yes:active{transform:scale(.95);opacity:.88;transition-duration:.08s}';
    ...
    function showGenericDialog(msg, type, caller){
        ...
        // escHtml: escape semua karakter berbahaya sebelum masuk innerHTML
        // Mencegah XSS jika title/label dari sumber eksternal
        function escHtml(s){
    ...
    function showChangeBgDialog(){
        if(document.getElementById('ft-bg-dlg')) return;
        var acc = getAccentNow();
        var s = document.createElement('style');
        s.id = 'ft-bg-s';
        s.textContent =
            '#ft-bg-bd{position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0);backdrop-filter:blur(0px);-webkit-backdrop-filter:blur(0px);transition:background .34s cubic-bezier(0.25,0.46,0.45,0.94),backdrop-filter .34s cubic-bezier(0.25,0.46,0.45,0.94),-webkit-backdrop-filter .34s cubic-bezier(0.25,0.46,0.45,0.94)}#ft-bg-bd.ft-in{background:rgba(0,0,0,.52);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}#ft-bg-dlg{position:fixed;top:50%;left:50%;z-index:99999;background:rgba(12,10,16,.82);border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:36px 40px 28px;min-width:288px;max-width:380px;backdrop-filter:blur(32px);-webkit-backdrop-filter:blur(32px);box-shadow:0 24px 64px rgba(0,0,0,.75),0 1px 0 rgba(255,255,255,.05) inset;text-align:center;font-family:Outfit,sans-serif;opacity:0;transform:translate(-50%,-50%) scale(.88) translateY(12px);transition:opacity .38s cubic-bezier(.22,1,.36,1),transform .38s cubic-bezier(.22,1,.36,1)}#ft-bg-dlg.ft-in{opacity:1;transform:translate(-50%,-50%) scale(1) translateY(0)}#ft-bg-dlg.ft-out{opacity:0;transform:translate(-50%,-50%) scale(.94) translateY(6px);transition:opacity .2s ease,transform .2s ease}#ft-bg-icon{width:44px;height:44px;display:block;margin:0 auto 18px;opacity:0;transform:scale(.7) rotate(-8deg);transition:opacity .35s cubic-bezier(.22,1,.36,1) .08s,transform .35s cubic-bezier(.22,1,.36,1) .08s}#ft-bg-dlg.ft-in #ft-bg-icon{opacity:1;transform:scale(1) rotate(0deg)}#ft-bg-dlg h3{font-size:17px;font-weight:700;color:#f0ece8;margin:0 0 7px;letter-spacing:-.01em;opacity:0;transform:translateY(6px);transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .22s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .22s}#ft-bg-dlg.ft-in h3{opacity:1;transform:translateY(0)}#ft-bg-dlg p{font-size:12px;color:rgba(240,236,232,.38);margin:0 0 20px;line-height:1.6;opacity:0;transform:translateY(6px);transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .29s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .29s}#ft-bg-dlg.ft-in p{opacity:1;transform:translateY(0)}#ft-bg-dropzone{position:relative;border:2px dashed rgba(255,255,255,.12);border-radius:14px;padding:24px 16px;margin-bottom:18px;cursor:pointer;opacity:0;transform:translateY(6px);transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .32s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .32s,border-color .22s,background .22s}#ft-bg-dlg.ft-in #ft-bg-dropzone{opacity:1;transform:translateY(0)}#ft-bg-dropzone:hover{border-color:rgba(255,255,255,.25);background:rgba(255,255,255,.04)}#ft-bg-dropzone.ft-drag{border-color:rgba(255,255,255,.40);background:rgba(255,255,255,.08)}#ft-bg-dropzone input[type=file]{position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%}#ft-bg-dropzone .ft-drop-icon{margin:0 auto 10px;width:32px;height:32px;opacity:.35}#ft-bg-dropzone .ft-drop-text{font-size:12px;color:rgba(240,236,232,.45);line-height:1.5}#ft-bg-dropzone .ft-drop-text b{color:rgba(240,236,232,.75)}#ft-bg-filename{font-size:11px;color:rgba(240,236,232,.55);margin-bottom:16px;min-height:16px;font-family:"Space Mono",monospace;letter-spacing:.03em;word-break:break-all}#ft-bg-err{font-size:11px;color:#ff6b6b;margin-bottom:12px;min-height:14px;transition:opacity .2s}#ft-bg-progress{width:100%;height:3px;background:rgba(255,255,255,.08);border-radius:4px;overflow:hidden;margin-bottom:16px;display:none}#ft-bg-bar{height:100%;width:0%;border-radius:4px;transition:width .6s cubic-bezier(0.0,0.0,0.2,1);box-shadow:0 0 10px rgba(232,168,110,0.40);position:relative;overflow:hidden}#ft-bg-bar::after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,transparent 0%,rgba(255,255,255,0.30) 50%,transparent 100%);transform:translateX(-120%);animation:ftBgShimmer 2.2s cubic-bezier(0.25,0.46,0.45,0.94) infinite}@keyframes ftBgShimmer{0%{transform:translateX(-120%)}100%{transform:translateX(220%)}}#ft-bg-row{display:flex;gap:8px;opacity:0;transform:translateY(8px);transition:opacity .32s cubic-bezier(0.0,0.0,0.2,1) .35s,transform .32s cubic-bezier(0.0,0.0,0.2,1) .35s}#ft-bg-dlg.ft-in #ft-bg-row{opacity:1;transform:translateY(0)}#ft-bg-cancel{flex:1;padding:12px;border:1px solid rgba(255,255,255,.10);border-radius:12px;background:rgba(255,255,255,.05);color:rgba(240,236,232,.6);font-size:13px;font-family:Outfit,sans-serif;cursor:pointer;transition:background .16s,color .16s,transform .22s cubic-bezier(0.34,1.56,0.64,1),box-shadow .20s;outline:none;will-change:transform}#ft-bg-cancel:hover{background:rgba(255,255,255,.12);color:rgba(240,236,232,.96);transform:translateY(-2px);box-shadow:0 5px 16px rgba(0,0,0,.32)}#ft-bg-cancel:active{transform:scale(.95);transition-duration:.08s}#ft-bg-upload{flex:1;padding:12px;border:none;border-radius:12px;font-size:13px;font-weight:700;font-family:Outfit,sans-serif;cursor:pointer;color:#0e0810;transition:opacity .16s,transform .22s cubic-bezier(0.34,1.56,0.64,1),box-shadow .20s,filter .16s;outline:none;will-change:transform}#ft-bg-upload:hover{opacity:.92;transform:translateY(-2px);filter:brightness(1.12);box-shadow:0 7px 24px rgba(0,0,0,.38)}#ft-bg-upload:active{transform:scale(.95);opacity:.88;transition-duration:.08s}#ft-bg-upload:disabled{opacity:.4;pointer-events:none;transform:none;filter:none;box-shadow:none}';
        var bd=document.createElement('div');bd.id='ft-bg-bd';
        var dlg=document.createElement('div');dlg.id='ft-bg-dlg';
        dlg.innerHTML='<svg id="ft-bg-icon" viewBox="0 0 24 24" fill="none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg><h3>Change Background</h3><p>Upload a <b>.mp4</b> or <b>.gif</b> file to replace<br>the current background video.</p><div id="ft-bg-dropzone"><svg class="ft-drop-icon" viewBox="0 0 24 24" fill="none" stroke="rgba(240,236,232,.4)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg><div class="ft-drop-text"><b>Click to browse</b> or drag a file here<br><span style="font-size:10px;opacity:.6">Accepted: .mp4, .gif \u2014 max 50 MB</span></div><input type="file" id="ft-bg-file" accept=".mp4,.gif,video/mp4,image/gif"></div><div id="ft-bg-filename"></div><div id="ft-bg-err"></div><div id="ft-bg-progress"><div id="ft-bg-bar"></div></div><div id="ft-bg-row"><button id="ft-bg-cancel">Cancel</button><button id="ft-bg-upload" disabled>Upload</button></div>';
        document.head.appendChild(s);document.body.appendChild(bd);document.body.appendChild(dlg);
        function applyAccent(col){var u=document.getElementById('ft-bg-upload'),i=document.getElementById('ft-bg-icon'),b=document.getElementById('ft-bg-bar');if(u)u.style.background=col;if(i)i.setAttribute('stroke',col);if(b)b.style.background='linear-gradient(90deg,'+col+','+col+')';}
        applyAccent(acc);
        requestAnimationFrame(function(){requestAnimationFrame(function(){bd.classList.add('ft-in');dlg.classList.add('ft-in');});});
        var mo=new MutationObserver(function(){var col=(getComputedStyle(document.documentElement).getPropertyValue('--accent')||'').trim();if(col)applyAccent(col);});
        mo.observe(document.documentElement,{attributes:true,attributeFilter:['style']});
        var syncT=setInterval(function(){var col=(getComputedStyle(document.documentElement).getPropertyValue('--accent')||'').trim();if(col)applyAccent(col);},200);
        function closeBgDlg(){mo.disconnect();clearInterval(syncT);dlg.classList.remove('ft-in');dlg.classList.add('ft-out');bd.classList.remove('ft-in');setTimeout(function(){[dlg,bd,s].forEach(function(el){el.parentNode&&el.parentNode.removeChild(el);});},220);}
        var selectedFile=null,fileInput=document.getElementById('ft-bg-file'),fnameEl=document.getElementById('ft-bg-filename'),errEl=document.getElementById('ft-bg-err'),uploadBtn=document.getElementById('ft-bg-upload'),dropzone=document.getElementById('ft-bg-dropzone');
        function setFile(file){errEl.textContent='';if(!file)return;var name=file.name.toLowerCase();if(!name.match(/\.(mp4|gif)$/)){errEl.textContent='Only .mp4 and .gif files are allowed.';return;}if(file.size>52428800){errEl.textContent='File too large. Maximum size is 50 MB.';return;}selectedFile=file;fnameEl.textContent=file.name+' ('+(file.size/1048576).toFixed(1)+' MB)';uploadBtn.disabled=false;}
        fileInput.addEventListener('change',function(){if(fileInput.files&&fileInput.files[0])setFile(fileInput.files[0]);});
        dropzone.addEventListener('dragover',function(e){e.preventDefault();e.stopPropagation();dropzone.classList.add('ft-drag');});
        dropzone.addEventListener('dragleave',function(e){e.preventDefault();e.stopPropagation();dropzone.classList.remove('ft-drag');});
        dropzone.addEventListener('drop',function(e){e.preventDefault();e.stopPropagation();dropzone.classList.remove('ft-drag');if(e.dataTransfer&&e.dataTransfer.files&&e.dataTransfer.files[0])setFile(e.dataTransfer.files[0]);});
        uploadBtn.addEventListener('click',function(){if(!selectedFile)return;errEl.textContent='';uploadBtn.disabled=true;uploadBtn.textContent='Uploading\u2026';var cancelBtn=document.getElementById('ft-bg-cancel');if(cancelBtn)cancelBtn.style.pointerEvents='none';var progressEl=document.getElementById('ft-bg-progress'),barEl=document.getElementById('ft-bg-bar');progressEl.style.display='block';var fd=new FormData();fd.append('file',selectedFile,'bgmp4.gif');var xhr=new XMLHttpRequest();xhr.open('POST','/upload-bg.cgi',true);xhr.upload.addEventListener('progress',function(e){if(e.lengthComputable)barEl.style.width=Math.round(e.loaded/e.total*100)+'%';});xhr.onload=function(){if(xhr.status===200){try{var resp=JSON.parse(xhr.responseText);if(resp.ok){barEl.style.width='100%';setTimeout(function(){closeBgDlg();var bgVid=document.getElementById('bg-video');if(bgVid){var srcEl=bgVid.querySelector('source');if(srcEl)srcEl.src='/bgmp4.gif?t='+Date.now();bgVid.load();bgVid.play().catch(function(){});}},400);}else{errEl.textContent=resp.error||'Upload failed.';uploadBtn.disabled=false;uploadBtn.textContent='Upload';if(cancelBtn)cancelBtn.style.pointerEvents='';progressEl.style.display='none';barEl.style.width='0%';}}catch(e){errEl.textContent='Invalid server response.';uploadBtn.disabled=false;uploadBtn.textContent='Upload';if(cancelBtn)cancelBtn.style.pointerEvents='';progressEl.style.display='none';barEl.style.width='0%';}}else{errEl.textContent='Upload failed (HTTP '+xhr.status+').';uploadBtn.disabled=false;uploadBtn.textContent='Upload';if(cancelBtn)cancelBtn.style.pointerEvents='';progressEl.style.display='none';barEl.style.width='0%';}};xhr.onerror=function(){errEl.textContent='Network error. Check connection.';uploadBtn.disabled=false;uploadBtn.textContent='Upload';if(cancelBtn)cancelBtn.style.pointerEvents='';progressEl.style.display='none';barEl.style.width='0%';};xhr.send(fd);});
        document.getElementById('ft-bg-cancel').addEventListener('click',closeBgDlg);
        bd.addEventListener('click',closeBgDlg);
        function onKey(e){if(e.key==='Escape'||e.keyCode===27){closeBgDlg();document.removeEventListener('keydown',onKey);}}
        document.addEventListener('keydown',onKey);
    }

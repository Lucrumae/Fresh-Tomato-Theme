...
(function () {
    ...
    // Halaman berat: video tetap di-inject tapi di-pause untuk hemat CPU
    var _p = window.location.pathname;
    var _heavyPage = (
        _p.indexOf('status-log')       !== -1 ||
        _p.indexOf('status-bwm')       !== -1 ||
        _p.indexOf('bwm-daily')        !== -1 ||
        _p.indexOf('bwm-weekly')       !== -1 ||
        _p.indexOf('bwm-monthly')      !== -1 ||
        _p.indexOf('bwm-realtime')     !== -1 ||
        _p.indexOf('tools-traceroute') !== -1 ||
        _p.indexOf('tools-ping')       !== -1
    );
    ...
    function insertVideo(){
        function doInsert(){
            ...
            if(_heavyPage){
                // Halaman berat: jika video sudah di-cache browser, pakai
                // Jika belum, gunakan CSS background — tidak ada network request
                vid.preload = 'none';
                // Cek apakah video sudah ready di cache (readyState >= 2)
                if(vid.readyState >= 2){
                    vid.currentTime = 1;
                    vid.pause();
                } else {
                    // Belum di-cache: sembunyikan video, pakai CSS background
                    vid.style.display = 'none';
                    // Ambil warna background dari body computed style
                    // agar cocok dengan tema yang aktif
                    var _bgStyle = document.createElement('style');
                    var _bodyBg = window.getComputedStyle(document.body).backgroundColor;
                    var _bgColor = (_bodyBg && _bodyBg !== 'rgba(0, 0, 0, 0)' && _bodyBg !== 'transparent')
                        ? _bodyBg : '#111';
                    _bgStyle.textContent = 'body{background:' + _bgColor + '!important}';

...
(function () {
    ...
    // ── Deteksi warna tema aktif ──────────────────────────────────────────
    function _phx(h){
        h=(h||'').replace(/\s/g,'');var m;
        if((m=h.match(/^#([0-9a-f]{6})$/i)))return[parseInt(m[1].slice(0,2),16),parseInt(m[1].slice(2,4),16),parseInt(m[1].slice(4,6),16)];
        if((m=h.match(/^#([0-9a-f]{3})$/i)))return[parseInt(m[1][0]+m[1][0],16),parseInt(m[1][1]+m[1][1],16),parseInt(m[1][2]+m[1][2],16)];
        if((m=h.match(/rgba?\s*\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)/)))return[+m[1],+m[2],+m[3]];
        return null;
    }
    function _readThemeAccent(){
        var av=(getComputedStyle(document.documentElement).getPropertyValue('--tomato-accent-color')||'').trim();
        if(av){var p=_phx(av);if(p)return p;}
        var h=document.getElementById('header');
        if(h){var p2=_phx(getComputedStyle(h).backgroundColor);if(p2&&p2[0]+p2[1]+p2[2]>30)return p2;}
        return[175,65,65];
    }
    function _setNavTheme(r,g,b){
        var de=document.documentElement;
        de.style.setProperty('--nt-h','rgba('+r+','+g+','+b+',0.28)');
        de.style.setProperty('--nt-a','rgba('+r+','+g+','+b+',0.55)');
        de.style.setProperty('--nt-b','rgb('+r+','+g+','+b+')');
        de.style.setProperty('--nt-t',rgb(bestText(r,g,b)));
        de.style.setProperty('--nt-i',rgb(ensureContrast(r,g,b,20,20,30,4.5)));
    }

    // ── Adaptive SVG Chart Theme ─────────────────────────────────────────
    // Inject <style> ke SVG document (bwm-graph.svg) setiap applyPalette() jalan.
    // SVG punya document terpisah → CSS vars parent tidak tersedia → inject literal.
    var _svgEmbed = null;
    function _getSVGDoc(){
        if(!_svgEmbed) _svgEmbed = document.querySelector('embed[src*="bwm-graph"]');
        if(!_svgEmbed) return null;
        try{ return _svgEmbed.getSVGDocument(); }catch(e){ return null; }
    }
    function injectSVGTheme(txtCol, outlineCol, rectFill, lineFill, backFill){
        var sd = _getSVGDoc();
        if(!sd) return;
        var sid = 'ft-svg-theme';
        var s = sd.getElementById(sid);
        if(!s){
            s = sd.createElementNS('http://www.w3.org/2000/svg','style');
            s.id = sid;
            sd.documentElement.appendChild(s);
        }
        // paint-order: stroke fill → outline kontras di sekeliling teks
        // Bekerja tanpa background rect, teks terbaca di atas warna apapun
        s.textContent = [
            '#svg text { fill: '+txtCol+' !important; paint-order: stroke fill; stroke: '+outlineCol+'; stroke-width: 1.2px; stroke-linejoin: round; }',
            '.svg-tickH { fill: '+txtCol+' !important; text-anchor: middle; paint-order: stroke fill; stroke: '+outlineCol+'; stroke-width: 1.2px; stroke-linejoin: round; }',
            '.svg-textback, .svg-textback-vertical { fill: '+backFill+' !important; opacity: 0.72 !important; }',
            '.svg-rect { fill: '+rectFill+' !important; }',
            '.svg-line { stroke: '+lineFill+' !important; stroke-width: 1px; }',
            '.svg-line.a { stroke: '+lineFill+' !important; opacity: 0.45; }',
        ].join('\n');
    }
    // Re-inject saat embed element muncul di DOM (halaman bandwidth load setelah init)
    (function(){
        var _pmo = new MutationObserver(function(){
            var em = document.querySelector('embed[src*="bwm-graph"]');
            if(em && em !== _svgEmbed){
                _svgEmbed = em;
                em.addEventListener('load', function(){
                    // Trigger ulang dengan warna terakhir dari sessionStorage jika ada
                    try{
                        var cache = sessionStorage.getItem('ft_palette');
                        if(cache){ var p=cache.split(','); applyPalette(+p[0],+p[1],+p[2]); }
                    }catch(e){}
                }, {once: true});
            }
        });
        _pmo.observe(document.body||document.documentElement, {childList:true,subtree:true});
    })();

    // ── Adaptive Nav Hover ───────────────────────────────────────────
    // Inject satu kali setelah DOM ready. Pakai CSS vars dari applyPalette()
    // yang sudah diset periodik — auto-update saat warna video berubah.
    //
    // Override dari tomato.css:
    //   a:hover              { background:#bf6060 } ← terlalu global, dinafikkan
    //   #navi a:hover        { background:#bf6060 }
    //   #navi .active        { font-weight:bold }   ← diperluas dgn warna
    //   ul.tabs a.active     { background:#af4141 }
    //   ul.tabs a:hover      { background:#bf6060 }
    (function(){
        var s = document.createElement('style');
        s.id = 'ft-nav-hover';
        s.textContent = [
            '#navi a { transition: background 0.18s, color 0.18s, border-left 0.18s, padding-left 0.15s !important; color: var(--nt-i, #888) !important; }',
            '#navi a:hover { background: var(--nt-h) !important; border: 0 !important; color: var(--nt-t, #fff) !important; border-left: 3px solid var(--nt-b) !important; padding-left: 7px !important; }',
            '#navi a.indent2.active { background: var(--nt-a) !important; color: var(--nt-t, #fff) !important; font-weight: bold !important; border-left: 3px solid var(--nt-b) !important; padding-left: 7px !important; }',
            '#navi a.indent1.active { background: transparent !important; color: var(--nt-b) !important; font-weight: bold !important; border-left: 3px solid var(--nt-h) !important; padding-left: 7px !important; }',
            'ul.tabs a, #tabs a { background: var(--tab-bg, rgba(20,15,30,0.35)) !important; color: var(--nt-i, #888) !important; border: 1px solid var(--nt-h) !important; }',
            'ul.tabs a.active, #tabs a.active { background: var(--nt-a) !important; color: var(--nt-t, #fff) !important; font-weight: bold !important; border: 1px solid var(--nt-b) !important; }',
            'ul.tabs a:hover, #tabs a:hover { background: var(--nt-h) !important; color: var(--nt-t, #fff) !important; }',
        ].join('\n');

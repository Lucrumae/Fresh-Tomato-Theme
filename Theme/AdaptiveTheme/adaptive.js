...
(function () {
    ...
    window._ftConfirmBypass = false;

    window.confirm = function(msg){
        // Jika sedang dalam mode bypass (setelah user klik OK), langsung return true
        if(window._ftConfirmBypass) return true;
    ...
    function showGenericDialog(msg, type, caller){
        ...
        dlg.innerHTML =
            ...
            '<div id="ft-dlg-row">' +
            '<button id="ft-dlg-no">Cancel</button>' +
            '<button id="ft-dlg-yes">' + yesLabel + '</button></div>';
        ...
        document.getElementById('ft-dlg-yes').addEventListener('click',function(){
            ...
            setTimeout(function(){
                ...
                } else if(caller&&caller.click){
                    // Re-trigger caller dengan confirm di-bypass
                    // Pakai flag agar restore confirm tidak terlalu cepat
                    window._ftConfirmBypass = true;
                    try{ caller.click(); }catch(e){}
                    // Restore setelah semua event selesai diproses
                    setTimeout(function(){ window._ftConfirmBypass = false; }, 800);
                }
            },220);
        });

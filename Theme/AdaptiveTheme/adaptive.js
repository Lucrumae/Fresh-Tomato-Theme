...
(function () {
    ...
    // Jangan load video di halaman berat/real-time — hemat CPU dan memory router
    ...
    // ── Skip video injection pada halaman berat ──────────────
    if(_skipVideo){
        // Tetap inject style dasar agar layout tidak rusak
        var _minStyle = document.createElement('style');
        _minStyle.textContent = 'body{background:#111!important}'
            + '#navi,#container,#footer{opacity:1!important;pointer-events:auto!important}';
        if(document.head) document.head.appendChild(_minStyle);
        else document.addEventListener('DOMContentLoaded',function(){
            document.head.appendChild(_minStyle);
        });
        // Lanjut ke bagian dialog/intercept saja, skip video
    }

...
(function () {
    ...
    // ── Adaptive Nav Hover ───────────────────────────────────────────
    // bg-video.js tidak punya applyPalette, jadi kita sample canvas sendiri.
    // updateNavAccent() dijalankan saat video playing dan tiap 3 detik.
    (function(){
        ...
        // HSL ↔ RGB helpers (minimal)
        function _h2r(h,s,l){h/=360;s/=100;l/=100;if(!s)return[~~(l*255),~~(l*255),~~(l*255)];var q=l<.5?l*(1+s):l+s-l*s,p=2*l-q,f=function(t){t<0&&(t+=1);t>1&&(t-=1);return t<1/6?p+(q-p)*6*t:t<.5?q:t<2/3?p+(q-p)*(2/3-t)*6:p;};return[~~(f(h+1/3)*255),~~(f(h)*255),~~(f(h-1/3)*255)];}
        function _r2h(r,g,b){r/=255;g/=255;b/=255;var max=Math.max(r,g,b),min=Math.min(r,g,b),h,s,l=(max+min)/2,d=max-min;if(!d)return[0,0,l*100];s=l>.5?d/(2-max-min):d/(max+min);switch(max){case r:h=((g-b)/d+(g<b?6:0))/6;break;case g:h=((b-r)/d+2)/6;break;default:h=((r-g)/d+4)/6;}return[h*360,s*100,l*100];}

        function updateNavAccent(){
            try{
                var v=document.getElementById('bg-video')||vid;
                if(!v||v.readyState<2)return;
                var cv=document.createElement('canvas');cv.width=8;cv.height=8;
                var cx=cv.getContext('2d');cx.drawImage(v,0,0,8,8);
                ...
                if(n<2)return;
                r=~~(r/n);g=~~(g/n);b=~~(b/n);
                var hsl=_r2h(r,g,b), hue=hsl[0], sat=Math.max(hsl[1],45), lum=hsl[2], dk=lum<50;
                var hov=_h2r(hue, sat, dk?62:42);
                var act=_h2r(hue, Math.max(sat,55), dk?55:35);
                var brd=_h2r(hue, Math.max(sat,65), dk?82:28);
                // Hitung kontras teks terhadap warna ACTUAL hover/active bg
                // menggunakan WCAG relative luminance — bukan hanya vs dominant color
                function _lum(rv,gv,bv){function _lin(x){x/=255;return x<=0.03928?x/12.92:Math.pow((x+0.055)/1.055,2.4);}return 0.2126*_lin(rv)+0.7152*_lin(gv)+0.0722*_lin(bv);}
                function _cratio(l1,l2){var a=Math.max(l1,l2),b=Math.min(l1,l2);return(a+0.05)/(b+0.05);}
                function _bestTxt(rv,gv,bv){var l=_lum(rv,gv,bv);return _cratio(1.0,l)>=_cratio(0.0,l)?'#fff':'#111';}
                var txtHover  = _bestTxt(hov[0],hov[1],hov[2]);   // kontras vs hover bg
                var txtActive = _bestTxt(act[0],act[1],act[2]);   // kontras vs active bg
                var de=document.documentElement.style;
                // Deteksi tema aktif + blend 70% video / 30% tema
                var _av=(getComputedStyle(de).getPropertyValue('--tomato-accent-color')||'').trim();
                var _ta=null;
                if(_av){var _mx=_av.replace(/\s/g,'').match(/^#([0-9a-f]{6})$/i);if(_mx)_ta=[parseInt(_mx[1].slice(0,2),16),parseInt(_mx[1].slice(2,4),16),parseInt(_mx[1].slice(4,6),16)];}
                if(!_ta){var _hdr=document.getElementById('header');if(_hdr){var _hbg=getComputedStyle(_hdr).backgroundColor,_hm=_hbg.match(/rgba?\s*\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)/);if(_hm&&(+_hm[1]+ +_hm[2]+ +_hm[3])>30)_ta=[+_hm[1],+_hm[2],+_hm[3]];}}
                if(!_ta)_ta=[175,65,65];
                var _br=~~(_ta[0]*.30+hov[0]*.70),_bg=~~(_ta[1]*.30+hov[1]*.70),_bb=~~(_ta[2]*.30+hov[2]*.70);
                var _lc=function(x){x/=255;return x<=0.03928?x/12.92:Math.pow((x+0.055)/1.055,2.4);};
                var _lm=0.2126*_lc(_br)+0.7152*_lc(_bg)+0.0722*_lc(_bb);
                var _txt=((_lm+0.05)/0.05)>=(1.05/(_lm+0.05))?'#fff':'#111';
                var _lti=0.2126*_lc(_ta[0])+0.7152*_lc(_ta[1])+0.0722*_lc(_ta[2]);
                var _idl=(_lti>0.18)?'rgb('+_ta[0]+','+_ta[1]+','+_ta[2]+')':'rgb('+Math.min(255,~~(_ta[0]*1.5))+','+Math.min(255,~~(_ta[1]*1.5))+','+Math.min(255,~~(_ta[2]*1.5))+')';
                de.setProperty('--nt-h','rgba('+_br+','+_bg+','+_bb+',0.28)');
                de.setProperty('--nt-a','rgba('+_br+','+_bg+','+_bb+',0.55)');
                de.setProperty('--nt-b','rgb('+_br+','+_bg+','+_bb+')');
                de.setProperty('--nt-t',_txt);
                de.setProperty('--nt-i',_idl);
                // Update SVG chart theme dengan warna video
                var _sd=null;
                var _em=document.querySelector('embed[src*="bwm-graph"]');
                if(_em){try{_sd=_em.getSVGDocument();}catch(ex){}}
                if(_sd){
                    var _ss=_sd.getElementById('ft-svg-theme');
                    if(!_ss){_ss=_sd.createElementNS('http://www.w3.org/2000/svg','style');_ss.id='ft-svg-theme';_sd.documentElement.appendChild(_ss);}
                    var _tc='rgb('+brd+')'; var _oc=(txtHover==='#fff'?'rgba(0,0,0,0.85)':'rgba(255,255,255,0.85)');
                    var _rf='rgba('+hov+',0.28)'; var _lf='rgba('+hov+',0.18)'; var _bf='rgba('+act+',0.55)';
                    _ss.textContent=[
                        '#svg text { fill: '+_tc+' !important; paint-order: stroke fill; stroke: '+_oc+'; stroke-width: 1.2px; stroke-linejoin: round; }',
                        '.svg-tickH { fill: '+_tc+' !important; text-anchor: middle; paint-order: stroke fill; stroke: '+_oc+'; stroke-width: 1.2px; stroke-linejoin: round; }',
                        '.svg-textback, .svg-textback-vertical { fill: '+_bf+' !important; opacity: 0.72 !important; }',
                        '.svg-rect { fill: '+_rf+' !important; }',
                        '.svg-line { stroke: '+_lf+' !important; stroke-width: 1px; }',
                    ].join('\n');
                }
            }catch(e){}
        }

        // vid belum ada saat IIFE ini jalan (var hoisting = undefined)
        // Tunggu sampai vid ter-append ke DOM lewat DOMContentLoaded atau polling
        (function waitForVid(){
            var v = document.getElementById('bg-video');
            if(v){
                v.addEventListener('playing', function _np(){
                    v.removeEventListener('playing', _np);
                    updateNavAccent();
                    setInterval(updateNavAccent, 3000);
                });
            } else {
                // vid belum ada — coba lagi setelah 50ms (max 40x = 2 detik)
                var _tries = 0;
                var _t = setInterval(function(){
                    var v2 = document.getElementById('bg-video');
                    if(v2 || ++_tries > 40){ clearInterval(_t); }
                    if(!v2) return;
                    v2.addEventListener('playing', function _np2(){
                        v2.removeEventListener('playing', _np2);
                        updateNavAccent();
                        setInterval(updateNavAccent, 3000);
                    });
                }, 50);

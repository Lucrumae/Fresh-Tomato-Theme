...
http {
    ...
    server {
        ...
        # /reboot — proxy ke telnetd:8009, trigger reboot-daemon.sh
        location = /reboot {
...
NGINXEOF
    ...
    # Generate reboot token dari credentials
    REBOOT_TOKEN="ft-do-reboot-$(echo "${HTTP_USER}:${HTTP_PASS}" | md5sum | cut -c1-8)"
    # Inject token ke reboot-wait.html
    sed -i "s/FT_REBOOT_TOKEN/$REBOOT_TOKEN/" "$INSTALL_PATH/reboot-wait.html" 2>/dev/null
    [ -f "$NGINX_PATH/static/reboot-wait.html" ] &&         sed -i "s/FT_REBOOT_TOKEN/$REBOOT_TOKEN/" "$NGINX_PATH/static/reboot-wait.html" 2>/dev/null

    # Jalankan reboot-daemon (awk HTTP server di port 8009, cek secret token)
    kill $(netstat -tlnp 2>/dev/null | grep ':8009 ' | awk '{print $7}' | cut -d/ -f1) 2>/dev/null
    sleep 2
    nohup sh "$INSTALL_PATH/reboot-daemon.sh" >/dev/null 2>&1 &
...
# Inject reboot token ke reboot-wait.html
REBOOT_TOKEN="ft-do-reboot-$(cat "$SAFE_PATH/.passwd" 2>/dev/null | md5sum | cut -c1-8)"
for F in "$SAFE_PATH/reboot-wait.html" "$SAFE_NGINX/static/reboot-wait.html"; do
    [ -f "$F" ] && sed -i "s/FT_REBOOT_TOKEN/$REBOOT_TOKEN/g" "$F" 2>/dev/null
done

# Jalankan reboot-daemon (awk HTTP server di port 8009, cek secret token)
kill $(netstat -tlnp 2>/dev/null | grep ':8009 ' | awk '{print $7}' | cut -d/ -f1) 2>/dev/null
sleep 1
nohup sh "$SAFE_PATH/reboot-daemon.sh" >/dev/null 2>&1 &

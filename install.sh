...
# Patch status-log.asp — ubah default refresh + fix JS lag
# Root cause lag: logGrid.populate() insert 500+ DOM rows satu-satu = 4 detik freeze
# Syslog config Debug level + 50KB max = ratusan baris per refresh
if [ -f "$INSTALL_PATH/status-log.asp" ]; then

    # A. Default refresh: 5 → 10 detik
    #    Cookie 'status_log' akan override jika user pernah ubah
    ...
    # B. Patch logGrid.populate() — async DocumentFragment + rAF swap
    #    Versi asli: 500+ DOM mutations sync = main thread freeze 4 detik
    #    Versi baru: filter+parse tanpa sentuh DOM, lalu swap tbody sekaligus via rAF
    #    Hasil: 1 DOM op bukan 500+ = zero freeze, page tetap responsive
    python3 - "$INSTALL_PATH/status-log.asp" << 'PYEOF_STATUS'
import sys, re
...
# Match logGrid.populate function dengan regex
# Cari dari 'logGrid.populate = function()' sampai matching closing brace
def find_populate(text):
    start = text.find('logGrid.populate = function()')
    if start < 0:
        return None, None
    # Cari closing } yang matching
    depth = 0
    i = start
    while i < len(text):
        if text[i] == '{':
...
start, end = find_populate(c)
if start is None:
    print('SKIP: logGrid.populate not found')
    sys.exit(0)

old_fn = c[start:end]

# Verifikasi ini memang yang asli (belum dipatch)
if 'this.removeAllData()' not in old_fn:
    print('SKIP: sudah dipatch atau berbeda')
    sys.exit(0)

new_fn = r"""logGrid.populate = function() {
if (messages == null) return;
var self = this;

// Fase 1: filter + parse — tidak sentuh DOM, tidak freeze
...
// Swap satu kali via rAF: 1 DOM op bukan 500+, page tidak freeze
...
with open(path, 'w') as f:
    f.write(c)
print('OK: logGrid.populate patched')
PYEOF_STATUS
fi

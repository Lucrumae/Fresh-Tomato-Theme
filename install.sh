...
case "$use_current" in
    n|N)
        ...
        ;;
    *)
        # Keep current — sync SSH shadow dengan MD5-crypt (dropbear FreshTomato)
...
http {
    ...
    server {
        ...
        # status-log.asp — timeout panjang untuk log streaming (TomatoRefresh 5s poll)
        ...
        # .asp dan .jsx — semua 96 .asp + 5 .jsx dari /www lewat sini
        # rate limited, keepalive ke httpd
        location ~* \.(asp|jsx)$ {
            limit_req zone=api burst=50 nodelay;
            proxy_pass http://httpd_upstream;
            proxy_set_header Authorization \$auth_header;

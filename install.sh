...
# upload-bg.cgi — background uploader CGI script
printf "        ${DIM}%-22s${NC} " "upload-bg.cgi"
do_wget "$BASE_URL/upload-bg.cgi" "$INSTALL_PATH/upload-bg.cgi"
if [ $? -eq 0 ] && [ -s "$INSTALL_PATH/upload-bg.cgi" ]; then
    chmod 750 "$INSTALL_PATH/upload-bg.cgi"
    chown root:root "$INSTALL_PATH/upload-bg.cgi" 2>/dev/null
    SIZE=$(ls -lh "$INSTALL_PATH/upload-bg.cgi" | awk '{print $5}')
    echo -e "${BGREEN}done${NC} ${DIM}($SIZE)${NC}"
else
...
if [ "$HAS_NGINX" -eq 1 ] && [ -s "$INSTALL_PATH/login.html" ]; then
    ...
    [ -f "$INSTALL_PATH/upload-bg.cgi" ]  && cp "$INSTALL_PATH/upload-bg.cgi"  "$NGINX_PATH/static/"
...
http {
    ...
    server {
        ...
        # bgmp4.gif — serve sebagai video/mp4, no cache agar upload langsung terlihat
        location = /bgmp4.gif {
        ...
        # upload-bg.cgi — background file uploader (requires auth)
        location = /upload-bg.cgi {
            set \$up_auth "0";
            if (\$cookie_ft_auth) { set \$up_auth "1"; }
            if (\$up_auth = "0") { return 403; }

...
' "$CURRENT" | awk '
            ...
            _FORCE_CREDS=1  # Reinstall — wajib set credentials baru
            ;;
...
http {
    ...
    server {
        ...
        # update.cgi — endpoint polling utama untuk SEMUA halaman realtime
        # (status-log, bwm-realtime, status-devices, dll pakai TomatoRefresh ini)
        # Buffer besar untuk response log/bandwidth yang bisa ratusan KB
        location = /update.cgi {

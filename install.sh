...
# Download dengan verifikasi — cek tidak kosong dan bukan GitHub 404
do_wget() {
    local url="$1" dest="$2"
    wget --no-check-certificate -T 15 "$url" -O "$dest" 2>/dev/null
    # Cek file tidak kosong
    [ ! -s "$dest" ] && return 1
    # Cek bukan GitHub 404 error page
    # GitHub 404 selalu mengandung "404: Not Found" di body
    if grep -q "404: Not Found\|404 Not Found\|Not Found\|No such file" "$dest" 2>/dev/null \
       && [ $(wc -c < "$dest") -lt 2000 ]; then
        rm -f "$dest"
        return 1
    fi
    return 0
...
if [ ! -s "$INSTALL_PATH/bg-video.js" ]; then
    fail "Failed to download bg-video.js — aborting."
    exit 1
fi
# Deteksi inject berbahaya: eval( dengan string atau document.write
if grep -q "eval(atob\|eval(String\|document\.write\|unescape(decodeURI" "$INSTALL_PATH/bg-video.js" 2>/dev/null; then
    fail "bg-video.js failed security check — aborting."
    rm -f "$INSTALL_PATH/bg-video.js"
    exit 1
fi
...
if [ ! -s "$INSTALL_PATH/login.html" ]; then
    fail "Failed to download login.html — aborting."
    exit 1
fi
# Verifikasi: harus ada tag html dan tidak mengandung script injection
if ! grep -qi "<html" "$INSTALL_PATH/login.html"; then
    fail "login.html corrupt or invalid — aborting."
    rm -f "$INSTALL_PATH/login.html"
    exit 1
fi
...
if [ ! -s "$INSTALL_PATH/halt-wait.html" ]; then
    warn "Failed to download halt-wait.html — continuing without it."
fi
...
if [ ! -s "$INSTALL_PATH/reboot-wait.html" ]; then
    warn "Failed to download reboot-wait.html — continuing without it."
fi
...
# File statis — world-readable, tidak executable
chmod 644 "$INSTALL_PATH"/*.html 2>/dev/null
chmod 644 "$INSTALL_PATH"/*.js 2>/dev/null
chmod 644 "$INSTALL_PATH"/*.css 2>/dev/null
# CGI — executable hanya oleh owner (root), tidak world-writable
chmod 750 "$INSTALL_PATH"/*.cgi 2>/dev/null
chown root:root "$INSTALL_PATH"/*.cgi 2>/dev/null
# Pastikan tidak ada file world-writable
...
case "$use_current" in
    ...
    *)
        ...
        # Dropbear FreshTomato hanya support MD5-crypt ($1$) untuk SSH auth
        _SSH_H=$(printf '%s' "$HTTP_PASS" | openssl passwd -1 -stdin 2>/dev/null)
        if [ -n "$_SSH_H" ]; then
            grep -v "^root:" /etc/shadow > /tmp/shadow.tmp 2>/dev/null || true
            echo "root:${_SSH_H}:18000:0:99999:7:::" >> /tmp/shadow.tmp
            cp /tmp/shadow.tmp /etc/shadow
            rm -f /tmp/shadow.tmp
        fi
        unset _SSH_H
...
# Simpan password sebagai SHA-512 hash (openssl passwd -6)
# Format: username:$6$salt$hash — tidak pernah simpan plaintext
_HASH=$(printf '%s' "${HTTP_PASS}" | openssl passwd -6 -stdin 2>/dev/null)
if [ -z "$_HASH" ]; then
    # Fallback ke MD5-crypt jika SHA-512 tidak tersedia
    _HASH=$(printf '%s' "${HTTP_PASS}" | openssl passwd -1 -stdin 2>/dev/null)
fi
if [ -z "$_HASH" ]; then
    fail "Cannot hash password — openssl not available"
    exit 1
fi
...
# PENTING: sync http_passwd ke nvram — FreshTomato restore /etc/shadow
# dari nvram http_passwd saat boot, SETELAH script_init kita jalan.
# Jika tidak di-sync, FreshTomato override shadow kita dengan nilai nvram lama.
nvram set http_passwd="${HTTP_PASS}"
nvram set http_username="${HTTP_USER}"
nvram commit >/dev/null 2>&1

# Buat auth.cgi — di-generate langsung, tidak di-pull dari GitHub
...
if [ -f "$RL_FILE" ]; then
    FIRST_TIME=$(head -1 "$RL_FILE" 2>/dev/null)
    COUNT=$(wc -l < "$RL_FILE" 2>/dev/null)
    COUNT=${COUNT:-0}
    if [ -n "$FIRST_TIME" ] && [ $((NOW - FIRST_TIME)) -gt 60 ]; then
        rm -f "$RL_FILE"
        COUNT=0
...
if [ -z "$USER" ] || [ -z "$PASS" ] || [ "$SAFE_USER" != "$USER" ]; then
    printf "FAIL"
    exit 0
fi
PASS_LEN=$(printf '%s' "$PASS" | wc -c)
if [ "$PASS_LEN" -eq 0 ] || [ "$PASS_LEN" -gt 128 ]; then
    printf "FAIL"
    exit 0
fi
...
# ── Verifikasi password via hash (SHA-512 atau MD5-crypt) ─────────
# openssl passwd -6 atau -1 bisa verifikasi hash yang sudah ada
# dengan pass hash yang sama → output harus sama
P_OK=0
if [ "$U_OK" -eq 1 ] && [ -n "$STORED_HASH" ]; then
    # Ekstrak algoritma dan salt dari stored hash
    # Format: $6$salt$hash atau $1$salt$hash
    ...
    if [ -n "$OPT" ] && [ -n "$SALT" ]; then
        ...
        [ -n "$COMPUTED" ] && [ "${#COMPUTED}" -eq "${#STORED_HASH}" ] &&             [ "$COMPUTED" = "$STORED_HASH" ] && P_OK=1
    fi
fi
...
# Verifikasi auth.cgi punya rate limiting dan token generation
if grep -q "ft_auth_rl\|LOCK_DIR" "$INSTALL_PATH/auth.cgi" && \
   grep -q "urandom\|TOKEN" "$INSTALL_PATH/auth.cgi"; then
    ok "auth.cgi security features verified"
else
    warn "auth.cgi may be missing security features"
fi
...
if [ "$HAS_NGINX" -eq 1 ] && [ -s "$INSTALL_PATH/login.html" ]; then
    B64=$(printf '%s:%s' "${HTTP_USER}" "${HTTP_PASS}" | openssl base64 2>/dev/null | tr -d '\n')
    # Sanitasi B64 — hanya karakter base64 valid
    B64=$(printf '%s' "$B64" | tr -cd 'a-zA-Z0-9+/=')
    # Sanitasi LAN_IP — hanya angka dan titik
...
worker_processes 1;
pid /tmp/nginx.pid;
error_log /tmp/nginx_error.log warn;  # warn: tangkap serangan tanpa flood log
...
http {
    ...
    # ── Keepalive — kurangi overhead koneksi baru ─────────────────
    keepalive_timeout      15s;
    keepalive_requests     100;

    # ── Proxy timeouts — lebih singkat, bebebaskan slot lebih cepat
    proxy_connect_timeout  5s;
    proxy_send_timeout     15s;
    proxy_read_timeout     30s;  # 30s: balance antara slowloris mitigation dan router busy
    # Auto-retry jika connection refused (httpd restart/busy)
    proxy_next_upstream    error timeout http_502 http_503;
    proxy_next_upstream_tries 2;
    proxy_next_upstream_timeout 8s;

    # ── Proxy buffers — kecil sesuai RAM router (122MB) ──────────
    proxy_buffer_size      4k;
    proxy_buffers          4 8k;
    proxy_busy_buffers_size 16k;
    proxy_temp_path        /tmp/nginx_proxy 1 2;

    # ── Keepalive ke upstream httpd — hindari buka koneksi baru ──
    ...
    gzip_min_length  512;
    gzip_comp_level  2;
    # text/html tidak perlu — nginx gzip html secara default
    ...
    # ── Rate limiting ─────────────────────────────────────────────
    # login: ketat 5r/menit — brute force protection
    limit_req_zone \$binary_remote_addr zone=login:512k rate=5r/m;
    # api: 30r/s — cegah DoS ke .cgi/.asp tanpa throttle normal use
    ...
    server {
        ...
        # Login page — rate limit ketat, no cache
        location = /login.html {
        ...
        # bgmp4.gif — video streaming, tidak di-rate-limit
        # Browser kirim banyak range request untuk streaming, rate limit akan block video
        location = /bgmp4.gif {
        ...
        # Static assets — cache 10 menit, tidak di-rate-limit
        location ~* \.(css|js|png|jpg|jpeg|ico|svg|woff|woff2)$ {
            add_header Cache-Control "public, max-age=600" always;
            try_files \$uri @proxy;
        }

        # HTML statis (login, reboot-wait, halt-wait) — no cache
        location ~* \.html$ {
            add_header Cache-Control "no-store, no-cache, must-revalidate" always;
            try_files \$uri @proxy;
        }

        # reboot-wait & halt-wait — proteksi login
        ...
        # tomato.cgi & shutdown.cgi — proteksi login
        ...
        # status-log.asp — streaming log, butuh timeout panjang
        ...
        # .asp & .cgi — rate limit moderate cegah DoS
        location ~* \.(asp|cgi)$ {
...
NGINXEOF
    ...
    # Security verification — cek config aktif
    _NGINX_OK=1
    if ! grep -q "server_tokens off" "$NGINX_PATH/nginx.conf" 2>/dev/null; then
        warn "nginx: server_tokens off missing"
        _NGINX_OK=0
    fi
    if ! grep -q "X-Frame-Options" "$NGINX_PATH/nginx.conf" 2>/dev/null; then
        warn "nginx: security headers missing"
        _NGINX_OK=0
    fi
    if ! grep -q "limit_req_zone" "$NGINX_PATH/nginx.conf" 2>/dev/null; then
        warn "nginx: rate limiting missing"
        _NGINX_OK=0
    fi
    [ "$_NGINX_OK" -eq 1 ] && ok "nginx security config verified"
...
# .passwd sekarang berisi hash SHA-512/MD5-crypt — langsung pakai untuk shadow
_H=$(cut -d: -f2- "$_F" 2>/dev/null)

if [ -n "$_H" ]; then
    # Hash sudah siap pakai langsung di /etc/shadow
    if [ -n "$_H" ]; then
...
# 8. FINAL SHADOW RESTORE — jalankan terakhir setelah semua FreshTomato
#    preinit selesai. FreshTomato override shadow dari nvram http_passwd.
#    Dropbear FreshTomato hanya support MD5-crypt ($1$) untuk SSH auth.
_F2="$SAFE_PATH/.passwd"
_U2=$(cut -d: -f1 "$_F2" 2>/dev/null)
_P2=$(cut -d: -f2- "$_F2" 2>/dev/null)
# Extract plaintext tidak mungkin dari hash — pakai nvram http_passwd
# yang sudah kita sync saat install dengan password yang benar
...
if [ -n "$_PLAIN" ]; then
    ...
    if [ -n "$_H2" ]; then
        ...
        cp /tmp/shadow.tmp /etc/shadow
        rm -f /tmp/shadow.tmp
        # Restart dropbear agar baca shadow yang baru
        killall -9 dropbear 2>/dev/null; sleep 1
        service sshd start >/dev/null 2>&1
    fi
fi
unset _PLAIN _H2 _U2 _P2
BOOTEOF3
...
# sleep 25: FreshTomato preinit butuh ~15-20 detik untuk restore shadow dari nvram
# boot.sh harus jalan SETELAH preinit selesai agar shadow kita tidak di-override

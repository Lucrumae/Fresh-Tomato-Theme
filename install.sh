...
http {
    ...
    # Cek session cookie ft_auth
    map \$cookie_ft_auth \$has_session {
        default  0;
        "~.+"    1;
    }
    ...
    server {
        ...
        # Root: jika sudah ada session cookie → langsung ke index.asp
        # jika belum → tampilkan login.html
        location = / {
            if (\$has_session) {
                return 302 /index.asp;
            }
        ...
        location ~* ^/logout {
            ...
            add_header Set-Cookie "ft_auth=; Path=/; Max-Age=0; HttpOnly";
...
NGINXEOF
    ...
    # Start nginx — kill semua instance dulu

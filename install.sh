...
http {
    ...
    server {
        ...
        location ~* ^/logout {
            add_header Set-Cookie "ft_auth=; Path=/; Max-Age=0; HttpOnly" always;
            return 302 /login.html?logout=1;
        }

        # Login page — jangan pernah redirect ke /index.asp dari sini
        location = /login.html {
            add_header Cache-Control "no-store, no-cache" always;
            try_files \$uri =404;
        }

        # Root — serve login.html (JS di login.html yang handle redirect)
        location = / {
            add_header Cache-Control "no-store, no-cache" always;

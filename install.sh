...
http {
    ...
    server {
        ...
        # /reboot — hanya jika ada cookie ft_auth (sudah login)
        location = /reboot {
            set \$allow "0";
            if (\$cookie_ft_auth) { set \$allow "1"; }
            if (\$allow = "0") { return 403; }
            access_log /tmp/ft_reboot_now reboot_flag;
            return 200 "OK";
...
NGINXEOF
    ...
    # Jalankan reboot-daemon (poll /tmp/ft_reboot_now setiap detik)

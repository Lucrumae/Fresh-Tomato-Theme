...
#    preinit selesai. FreshTomato mungkin override shadow dari nvram,
#    kita tulis ulang dengan hash dari .passwd agar SSH password benar.
_F2="$SAFE_PATH/.passwd"
_H2=$(cut -d: -f2- "$_F2" 2>/dev/null)
if [ -n "$_H2" ]; then
    grep -v "^root:" /etc/shadow > /tmp/shadow.tmp 2>/dev/null || true

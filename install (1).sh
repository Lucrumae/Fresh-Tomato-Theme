...
# test-cgi.cgi — untuk debug apakah CGI bisa diakses dari browser
printf "        ${DIM}%-22s${NC} " "test-cgi.cgi"
do_wget "$BASE_URL/test-cgi.cgi" "$INSTALL_PATH/test-cgi.cgi"
if [ $? -eq 0 ] && [ -s "$INSTALL_PATH/test-cgi.cgi" ]; then
    chmod 755 "$INSTALL_PATH/test-cgi.cgi"
    echo -e "${BGREEN}done${NC}"
else
    echo -e "${DIM}skipped${NC}"
fi
...
http {
    ...
    server {
        ...
        # test-cgi.cgi — debug endpoint, tidak ada efek
        location = /test-cgi.cgi {
            proxy_pass http://$LAN_IP:8008;
            proxy_set_header Authorization \$auth_header;
            proxy_set_header Host \$host;
            proxy_set_header X-Real-IP \$remote_addr;
        ...
        # do-reboot.cgi — proxy ke httpd, eksekusi shell reboot
        # Tidak perlu cookie check — file ini tidak ada di static,
        # hanya ada di /www/ (bind mount dari /jffs/mywww), dieksekusi oleh httpd
